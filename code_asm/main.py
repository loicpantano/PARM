...
def parse_asm_into_machine_code(asm: str) -> str:
    ...
    def __lsls(args: list, instruction_number: int = None) -> str:
        ...
        if '#' in args[-1]:
            return "00000" + __get_immediate(args[-1], 5) + __get_register(args[1]) + __get_register(args[0])
        return "0100000010" + __get_register(args[1]) + __get_register(args[0])
    ...
    def __lsrs(args: list, instruction_number: int = None) -> str:
        ...
        if '#' in args[-1]:
            return "00001" + __get_immediate(args[-1], 5) + __get_register(args[1]) + __get_register(args[0])
        return "0100000011" + __get_register(args[1]) + __get_register(args[0])
    ...
    def __asrs(args: list, instruction_number: int = None) -> str:
        ...
        if '#' in args[-1]:
            return "00010" + __get_immediate(args[-1], 5) + __get_register(args[1]) + __get_register(args[0])
        return "0100000100" + __get_register(args[1]) + __get_register(args[0])
    ...
    def __adds(args: list, instruction_number: int = None) -> str:
        ...
        if '#' in args[-1]:
            return "0001110" + __get_immediate(args[-1], 3) + __get_register(args[1]) + __get_register(args[0])

        return "0001100" + __get_register(args[2]) + __get_register(args[1]) + __get_register(args[0])
    ...
    for instruction in parsed_asm_code:
        ...
        fun = locals()["__" + keyword.lower()]
        binary = fun(instruction[1:], i)
        machine_code += __convert(binary, 2,
...
def main() -> None:
    ...
    for asm_file in asm_files:
        ...
        with open(asm_file, "r") as f:
            asm_code = f.read()
            machine_code = parse_asm_into_machine_code(asm_code)
